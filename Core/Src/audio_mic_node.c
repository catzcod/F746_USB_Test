 ...
 */
int8_t AUDIO_MEMS_MicInit(AUDIO_Description_t *audio_description, AUDIO_Session_t *session_handle, uint32_t node_handle)
{
	AUDIO_MicNode_t *mic;
...
//  mic->MicGetReadCount          = AUDIO_MicGetLastReadCount;
	...
	audio_description->resolution, audio_description->channels_count);
...
static void AUDIO_MicFillDataToBuffer(uint32_t pdm_offset)
{
  ...
  counter = ++AUDIO_MicStatsCounter;
	...
	{ /* first stop the Microphone */
		...
		BSP_AUDIO_IN_Init(AUDIO_MicHandler->node.audio_description->frequency, AUDIO_MicHandler->node.audio_description->resolution,
	...
	else
	{
		if (AUDIO_MicHandler->node.state == AUDIO_NODE_STARTED)
		{
			...
			{
				AUDIO_MicHandler->node.session_handle->SessionCallback(AUDIO_OVERRUN, (AUDIO_Node_t*) AUDIO_MicHandler, AUDIO_MicHandler->node.session_handle);
			}
			BSP_AUDIO_IN_PDMToPCM((uint16_t*) &AUDIO_MicHandler->specific.pdm_buff[pdm_offset],
					(uint16_t*) (AUDIO_MicHandler->buf->data + AUDIO_MicHandler->buf->wr_ptr), AUDIO_MicHandler->specific.pdm_tmp_buff,
					AUDIO_MicHandler->specific.pdm_packet_size);
			/* to change to support other resolution */
			/* check for overflow */
#if ((USB_AUDIO_CONFIG_RECORD_RES_BIT) != 16)
			AUDIO_DoPadding(AUDIO_MicHandler->buf->data + AUDIO_MicHandler->buf->wr_ptr, AUDIO_MicHandler->buf->data + AUDIO_MicHandler->buf->wr_ptr,
					AUDIO_MicHandler->packet_length);
