...
{
	USB_DISABLE_EP_BEFORE_CLOSE(ep_addr); //HAL_PCD_EP_Close(pdev->pData, ep_addr);
	HAL_PCD_EP_Close(pdev->pData, ep_addr); //USB_CLEAN_EP_AFTER_CLOSE(ep_addr);
	return USBD_OK;
}
...
static USBD_StatusTypeDef USBD_LL_Setup_Fifo(void)
{
	...
	uint16_t tx_fifo_size[5] =
	{ 0 };/* TX_FIFO allocation*/
 ...
 tx_fifo_size[USB_AUDIO_CONFIG_RECORD_EP_IN&0x7F] = (USBD_AUDIO_CONFIG_RECORD_MAX_PACKET_SIZE + 3U)/4;
	...
	rx_fifo_size += (5 * 1/*number of control endpoints*/+ 8 + 1/* for status information*/+ 2 * 6/*number of OUT endpoints*/+ 1/*for Global NAK*/);
