...
typedef enum
{
	USBD_AUDIO_DATA_EP, USBD_AUDIO_FEEDBACK_EP, USBD_AUDIO_INTERRUPT_EP
...
{ USBD_AUDIO_Init, USBD_AUDIO_DeInit, USBD_AUDIO_Setup, USBD_AUDIO_EP0_TxReady, USBD_AUDIO_EP0_RxReady, USBD_AUDIO_DataIn, USBD_AUDIO_DataOut, USBD_AUDIO_SOF,
		USBD_AUDIO_IsoINIncomplete, USBD_AUDIO_IsoOutIncomplete, USBD_AUDIO_GetCfgDesc, USBD_AUDIO_GetCfgDesc, USBD_AUDIO_GetCfgDesc,
...
static uint8_t USBD_AUDIO_SetInterfaceAlternate(USBD_HandleTypeDef *pdev, uint8_t as_interface_num, uint8_t new_alt)
{
	...
	else
	{
		...
		/* open data end point */
		USBD_LL_OpenEP(pdev, ep->ep_description.data_ep->ep_num,
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_INTERFACE:
		{
			for (int i = 0; i < haudio->aud_function.as_interfaces_count; i++)
			{
				...
				{
					USBD_CtlSendData(pdev, (uint8_t*) &(haudio->aud_function.as_interfaces[i].alternate), 1);
					return USBD_OK;
				}
			}
			USBD_CtlError(pdev, req);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	if (ep->open)
	{
      ...
      if(ep->ep_type==USBD_AUDIO_DATA_EP)
      {
		...
		ep->ep_description.data_ep->buf = ep->ep_description.data_ep->GetBuffer(ep->ep_description.data_ep->private_data, &ep->ep_description.data_ep->length);
		ep->tx_rx_soffn = USB_SOF_NUMBER();
		USBD_LL_Transmit(pdev, epnum | 0x80, ep->ep_description.data_ep->buf, ep->ep_description.data_ep->length);
#if USBD_SUPPORT_AUDIO_OUT_FEEDBACK 
     }
     else
	...
	if (haudio->last_control.request_target == AUDIO_UNIT_CONTROL_REQUEST)
	{
		...
		case USBD_AUDIO_CS_AC_SUBTYPE_FEATURE_UNIT:
		{
			...
			case USBD_AUDIO_CONTROL_FEATURE_UNIT_MUTE:
			{
				...
				{
					feature_control->SetMute(LOBYTE(haudio->last_control.wValue), haudio->last_control.data[0], ctl->private_data);
				}
				break;
			}
			case USBD_AUDIO_CONTROL_FEATURE_UNIT_VOLUME:
			{
				...
				case USBD_AUDIO_REQ_SET_CUR:
					if (feature_control->SetCurVolume)
					{
						...
						feature_control->SetCurVolume(LOBYTE(haudio->last_control.wValue), *tmpdata, ctl->private_data);
					}
					break;
				default:
     ...
     if(ep->ep_type==USBD_AUDIO_FEEDBACK_EP)
      {
        ...
        continue;
			...
			if (ep->ep_type == USBD_AUDIO_DATA_EP)
			{
				USBD_LL_Transmit(pdev, epnum, ep->ep_description.data_ep->buf, ep->ep_description.data_ep->length);
			}
			else
			{
				USBD_error_handler();
...
static uint8_t AUDIO_REQ(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	...
	if (!(req->bRequest & 0x80))
	{
		...
		USBD_CtlPrepareRx(pdev, haudio->last_control.data, req->wLength);
		return USBD_OK;
	}
	...
	case USBD_AUDIO_CS_AC_SUBTYPE_FEATURE_UNIT:
	{
		...
		case USBD_AUDIO_CONTROL_FEATURE_UNIT_MUTE:
		{
			...
			{
				feature_control->GetMute(LOBYTE(req->wValue), &haudio->last_control.data[0], ctl->private_data);
			}
		...
		case USBD_AUDIO_CONTROL_FEATURE_UNIT_VOLUME:
		{
			...
			case USBD_AUDIO_REQ_GET_CUR:
				...
				{
					feature_control->GetCurVolume(LOBYTE(req->wValue), (uint16_t*) haudio->last_control.data, ctl->private_data);
				}
				break;
 ...
 * @retval status
 */
uint8_t USBD_AUDIO_RegisterInterface(USBD_HandleTypeDef *pdev, USBD_AUDIO_InterfaceCallbacksfTypeDef *aifc)
{
	if (aifc != NULL)
	{
		pdev->pUserData = aifc;
